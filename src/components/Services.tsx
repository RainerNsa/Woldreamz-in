...
type ServiceCardProps = {
  ...
  description: string;
  className?: string;
  style?: React.CSSProperties; // Add the style prop to the type definition
};

const ServiceCard = ({ icon, title, description, className, style }: ServiceCardProps) => (
  <div 
    className={cn(
      "glass-card p-8 flex flex-col items-start group hover:scale-[1.02] cursor-pointer",
      className
    )}
    style={style} // Use the style prop
  >
    <div className="p-3 rounded-xl bg-woldreamz-50 text-woldreamz-blue mb-6 group-hover:bg-woldreamz-blue group-hover:text-white transition-colors duration-300">
      {icon}
    </div>
...
const Services = () => {
  ...
  return (
    <section className="py-16 md:py-24 bg-slate-50 relative overflow-hidden">
      <div className="absolute -top-[300px] -right-[300px] w-[600px] h-[600px] bg-woldreamz-50 rounded-full filter blur-3xl opacity-30"></div>
      
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        <div className="max-w-3xl mx-auto text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Our Services & Solutions</h2>
          <p className="text-slate-600">We offer comprehensive software solutions and educational technology services designed to meet your unique needs and drive success.</p>
        </div>
